...
open class InterceptorsReactiveObject : Interceptors, IUTSReactive<Interceptors> {
    ...
    override var requestReject: UTSArray<RequestRejectInterceptor>
        get() {
            return trackReactiveGet(__v_raw, "requestReject", __v_raw.requestReject, this.__v_isReadonly, this.__v_isShallow);
        }
        set(value) {
            if (!this.__v_canSet("requestReject")) {
                return;
            }
            val oldValue = __v_raw.requestReject;
            __v_raw.requestReject = value;
            triggerReactiveSet(__v_raw, "requestReject", oldValue, value);
        }
    ...
    override var responseReject: UTSArray<ResponseRejectInterceptor>
        get() {
            return trackReactiveGet(__v_raw, "responseReject", __v_raw.responseReject, this.__v_isReadonly, this.__v_isShallow);
        }
        set(value) {
            if (!this.__v_canSet("responseReject")) {
                return;
            }
            val oldValue = __v_raw.responseReject;
            __v_raw.responseReject = value;
            triggerReactiveSet(__v_raw, "responseReject", oldValue, value);
        }
    override var responseFail: UTSArray<ResponseFailInterceptor>
        get() {
            return trackReactiveGet(__v_raw, "responseFail", __v_raw.responseFail, this.__v_isReadonly, this.__v_isShallow);
        }
        set(value) {
            if (!this.__v_canSet("responseFail")) {
                return;
            }
            val oldValue = __v_raw.responseFail;
            __v_raw.responseFail = value;
            triggerReactiveSet(__v_raw, "responseFail", oldValue, value);
        }
}
...
val state = reactive(State(interceptors = Interceptors(request = utsArrayOf(), requestReject = utsArrayOf(), response = utsArrayOf(), responseXhr = utsArrayOf(), responseReject = utsArrayOf(), requestSync = null, requestRejectSync = null, responseSync = null, responseFail = utsArrayOf())));
...
open class InterceptorManager {
    ...
    constructor(){
        this.interceptors = Interceptors(request = utsArrayOf<RequestInterceptor>(), requestReject = utsArrayOf<RequestRejectInterceptor>(), response = utsArrayOf<ResponseInterceptor>(), responseXhr = utsArrayOf<ResponseXhrInterceptor>(), responseReject = utsArrayOf<ResponseRejectInterceptor>(), responseFail = utsArrayOf<ResponseFailInterceptor>(), requestSync = null, requestRejectSync = null, responseSync = null);
    }
        ...
        suspend fun async(): Any? {
            ...
            if (mergedOptions.needRequestInterceptor == true) {
                ...
                if (state.interceptors.requestSync != null) {
                    interceptorResult = await((state.interceptors.requestSync as RequestInterceptorSync)(mergedOptions));
                }
            ...
            return UTSPromise(fun(resolve, reject){
                ...
                this.requestTask = uni_request<Any>(RequestOptions(url = "" + mergedOptions.url, method = mergedOptions.method, data = mergedOptions.data, header = mergedOptions.header, dataType = mergedOptions.dataType, timeout = mergedOptions.timeout ?: 60000, responseType = mergedOptions.responseType, sslVerify = mergedOptions.sslVerify, withCredentials = mergedOptions.withCredentials, success = fun(res: RequestSuccess<Any>){
                    ...
                    if (mergedOptions.needResponseInterceptor == true) {
                        ...
                        state.interceptors.responseReject.forEach(fun(interceptor: ResponseRejectInterceptor){
                ...
                , fail = fun(fail: RequestFail){
                    ...
                    if (mergedOptions.needResponseInterceptor == true) {
                        state.interceptors.responseFail.forEach(fun(interceptor: ResponseFailInterceptor){
...
open class BatchRequestManager {
    ...
    open fun executeBatch(): UTSPromise<UTSArray<ExecuteBatchRequestResult>> {
        suspend fun async(): Any? {
            ...
            return settedResutls.map(fun(result): ExecuteBatchRequestResult {
        ...
        return UTSPromise(fun(resolve, reject) {
            kotlinx.coroutines.CoroutineScope(io.dcloud.uts.UTSAndroid.getDomCoroutineDispatcher()).async {
                try {
                    ...
                    resolve(if (result is UTSPromise<*>) {
                        @Suppress("UNCHECKED_CAST")
                        `await`(result as UTSPromise<UTSArray<ExecuteBatchRequestResult>>);
                    } else {
                        result as UTSArray<ExecuteBatchRequestResult>;
                    }
                    );
                }
                 catch (e: Throwable) {
